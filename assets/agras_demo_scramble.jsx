...
/* ═══════════════════════════════════════════════════════════
   ...
   Pool of 15 realistic Ancol BoD agenda scenarios.
   Scramble picks 5 randomly, always includes 1 Customer Data item.
   Each agenda is self-contained: discussion, decision, regulations, flags.
   ═══════════════════════════════════════════════════════════ */

const C = {
  bg: "#050D1A", surface: "#0A1628", card: "#0F2035",
  border: "rgba(47,111,126,0.2)",
  navy: "#002561", teal: "#2F6F7E", tealBright: "#3D9AAF", tealGlow: "rgba(47,111,126,0.3)",
  gold: "#D4A843", red: "#E74C3C", green: "#2ECC71",
  yellow: "#F1C40F", orange: "#E67E22",
  text: "#E8EDF5", textMid: "#7A8BA5", textDim: "#4A5B72", white: "#FFF",
};

// ═══════════════════════════════════════════════════════════
//  AGENDA POOL — 15 Self-Contained Scenarios
//  Each has: id, title, discussion, decision, type,
//            scribe (extracted block), regulations, flags
//  tag: "customer_data" guarantees inclusion
// ═══════════════════════════════════════════════════════════

const AGENDA_POOL = [
  // ── CUSTOMER DATA SCENARIOS (tag: customer_data) ──
  {
  ...
  // ── COASTAL DEVELOPMENT SCENARIOS ──
  {
    ...
    discussion: "Direktur Pengembangan Bisnis menyampaikan rencana pembangunan kawasan mixed-use seluas 8.5 hektar di area reklamasi pantai utara Ancol. Proyek mencakup hotel bintang 5, pusat perbelanjaan premium, dan residensial apartemen. Estimasi investasi Rp 2.8 triliun. Direktur Operasional memberikan penjelasan teknis mengenai kondisi lahan.",
  ...
  // ── CORPORATE FINANCE SCENARIOS ──
  {
    ...
    discussion: "Direktur Keuangan menyampaikan kebutuhan pendanaan ekspansi tiga tahun. Total kebutuhan Rp 4.5 triliun melalui rights issue Rp 2 triliun dan pinjaman Rp 2.5 triliun. Rasio 1:3, harga Rp 850/saham. Direktur Utama menyetujui prinsip dan meminta percepatan untuk Q2 2026.",
  ...
  },
  {
    ...
    regulations: [
      { tier: 2, reg: "UU No. 40/2007", title: "Perseroan Terbatas", article: "Pasal 37-39", relevant: "Buyback wajib keputusan RUPS kecuali kondisi tertentu dalam Pasal 37(2)", score: 0.95 },
  ...
  },
  {
    ...
    decision: "DIPUTUSKAN: Menyetujui pemberian pinjaman Rp 500 miliar kepada PT TIJA dengan bunga 4%/tahun, tenor 5 tahun. Ditugaskan kepada Direktur Keuangan untuk menyiapkan perjanjian pinjaman.",
    type: "DECISION",
  ...
  },
  {
    ...
    discussion: "Direktur Operasional meminta persetujuan untuk membuka wahana roller coaster baru \"Tsunami Ride\" untuk pengunjung pada libur Lebaran, meskipun sertifikasi keselamatan final dari Kemenaker belum diterbitkan. Sertifikasi sementara sudah diperoleh dan sertifikasi final diperkirakan selesai 2 minggu setelah Lebaran.",
    ...
    flags: [
      { tier: 4, severity: "LOW", color: "YELLOW", title: "Pengoperasian Wahana Tanpa Sertifikasi Final", summary: "Wahana rekreasi wajib memiliki sertifikat laik operasi sebelum dibuka untuk publik. Sertifikasi sementara tidak memenuhi ketentuan Permen Pariwisata No. 12/2019.", regulation: "Permen Pariwisata No. 12/2019 Pasal 8-11", reasoning: ["Keputusan membuka wahana sebelum sertifikasi final.", "Permen Pariwisata 12/2019 mewajibkan sertifikat laik operasi sebelum publik.", "Sertifikasi sementara tidak memenuhi ketentuan.", "Risiko keselamatan pengunjung dan tanggung jawab hukum."], action: "Tunda pembukaan Tsunami Ride sampai sertifikasi final diterbitkan Kemenaker. Risiko tanggung jawab hukum jika terjadi insiden.", confidence: 0.94, requiresHitl: false },
    ],
  },

  // ── GOVERNANCE & DISCLOSURE SCENARIOS ──
  {
    tag: "governance", id: "GV-1",
    title: "Penunjukan Direktur Baru Tanpa Melalui Fit & Proper Test",
    discussion: "Direktur Utama mengusulkan pengangkatan seorang kenalan pribadi sebagai Direktur Pengembangan Digital yang baru, tanpa melalui proses fit and proper test dan tanpa rekomendasi Komite Nominasi dan Remunerasi. Calon memiliki pengalaman di sektor teknologi namun tidak memiliki latar belakang di industri pariwisata atau BUMD.",
  ...
  // ── INFORMATIONAL (no flags) ──
  {
    ...
    decision: null, type: "INFORMATIONAL",
    scribe: null, regulations: [], flags: [],
  },
  {
    tag: "info", id: "IF-2",
...
function scrambleAgendas() {
  ...
  // Pick 1 customer data + 2 other decisions + 1 info + 1 more random decision
  ...
  // Renumber agendas
  return picked.map((a, i) => ({ ...a, displayId: `AGN-${i + 1}` }));
}

// ═══════════════════════════════════════════════════════════
//  GENERATE PROCESSING STEPS FROM SELECTED AGENDAS
// ═══════════════════════════════════════════════════════════
function generateSteps(agendas) {
  const steps = [];
  ...
  const riskScore = totalFlags === 0 ? 0 : Math.min((allFlags.reduce((s, f) => s + ({ CRITICAL: 0.4, HIGH: 0.3, MEDIUM: 0.2, LOW: 0.1 }[f.severity] || 0), 0) / 1.2), 1.0).toFixed(2);
  ...
  return { steps, riskScore: parseFloat(riskScore), riskLevel, totalFlags, redCount, decisionCount: decisions.length };
}
...
export default function App() {
  const [agendas, setAgendas] = useState(() => scrambleAgendas());
  const [phase, setPhase] = useState("document");
  ...
  const allFlags = agendas.flatMap((a, ai) => a.flags.map((f, fi) => ({ ...f, id: `FLG-${String(ai * 10 + fi + 1).padStart(3, "0")}`, blockTitle: a.scribe?.title || a.title })));
  ...
  useEffect(() => {
    ...
    if (phase === "processing" && step >= STEPS.length) setTimeout(() => setPhase("results"), 700);
  }, [phase, step, STEPS.length]);

  useEffect(() => { if (logRef.current) logRef.current.scrollTop = logRef.current.scrollHeight; }, [step]);

  const sevColor = s => ({ CRITICAL: C.red, HIGH: C.orange, MEDIUM: C.yellow, LOW: C.teal }[s] || C.textDim);
  const riskColor = riskLevel === "CRITICAL" || riskLevel === "HIGH" ? C.red : riskLevel === "MEDIUM" ? C.yellow : riskLevel === "LOW" ? C.teal : C.green;

  return (
    <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif", background: C.bg, color: C.text, minHeight: "100vh", maxWidth: "960px", margin: "0 auto" }}>
      <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700;800&family=Playfair+Display:wght@700;800&display=swap" rel="stylesheet" />
      <div style={{ position: "fixed", inset: 0, background: "radial-gradient(ellipse at 30% 20%, rgba(47,111,126,0.06) 0%, transparent 60%)", pointerEvents: "none" }} />
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 50, background: `${C.surface}EE`, backdropFilter: "blur(12px)", borderBottom: `1px solid ${C.border}`, padding: "10px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          {/* Phase Indicator */}
          {["document", "processing", "results"].map((p, i) => (
            <div key={p} style={{ display: "flex", alignItems: "center", gap: "3px" }}>
              <div style={{ width: 22, height: 22, borderRadius: "50%", background: phase === p ? C.teal : (["document", "processing", "results"].indexOf(phase) > i ? C.green : `${C.teal}20`), display: "flex", alignItems: "center", justifyContent: "center", fontSize: "9px", fontWeight: 700, color: phase === p || ["document", "processing", "results"].indexOf(phase) > i ? C.white : C.textDim, transition: "all 0.3s" }}>{i + 1}</div>
