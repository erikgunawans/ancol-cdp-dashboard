...
/* ═══════════════════════════════════════════════════════════
   ...
   Combined: Executive Summary + Architecture + Timeline + Options
   
   4 Pages:
   1. The Challenge & Solution
   2. How the System Works (Architecture)
   3. Accelerated Timeline (Parallel Gantt)
   4. Two Output Options + Next Steps
   
   Bilingual EN/ID — Garuda AI Dark Theme
   ═══════════════════════════════════════════════════════════ */

const C = {
  bg: "#06132A", surface: "#0B1D38", card: "#0F2340",
  border: "rgba(47,140,160,0.2)", borderGlow: "rgba(47,140,160,0.35)",
  navy: "#001D47", navyMid: "#0A2D5E",
  teal: "#2F8F9E", tealBright: "#38C7D6",
  gold: "#D4A843", red: "#E05545", green: "#3DBE7B",
  yellow: "#E8B73A", orange: "#E67E22",
  gemini: "#4285F4", react: "#61DAFB",
  text: "#E8EDF5", textMid: "#8DA0BA", textDim: "#4A6080", white: "#FFF",
};
...
const ArrowDown = ({ color = C.tealBright }) => (
    ...
    <motion.div {...fade(0.3)}>
      ...
      <div style={{ background: C.card, borderRadius: "14px", padding: "18px 22px" }}>
        <p style={{ fontSize: "11.5px", color: C.textMid, margin: "0 0 12px 0", lineHeight: 1.6 }}>
          {en ? <>AGRAS evaluates every decision using the <b style={{ color: C.text }}>Hierarchy of Truth</b> — a clear, consistent legal priority order:</> : <>AGRAS mengevaluasi setiap keputusan menggunakan <b style={{ color: C.text }}>Hierarchy of Truth</b> — urutan prioritas hukum yang jelas dan konsisten:</>}
        </p>
        {[
  ...
  <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
    ...
    <motion.div {...fade(0.5)} style={{ marginTop: "6px", background: `${C.navy}CC`, borderRadius: "10px", padding: "10px 16px", border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
      ...
      <div style={{ display: "flex", gap: "6px" }}>
        {["Gemini AI", "AES-256", en ? "ISO 27001" : "ISO 27001", en ? "Fully Managed" : "Terkelola Penuh"].map((b, i) => (
...
const Page3 = ({ en }) => {
  ...
  const streams = [
    { id: "A", l: en ? "Engineering" : "Rekayasa", s: en ? "Axiara.AI" : "Axiara.AI", c: C.tealBright,
    ...
    { id: "C", l: en ? "IT Division" : "Divisi IT", s: en ? "Ancol IT" : "IT Ancol", c: C.orange,
      bars: [{ a: 0, b: 1, t: en ? "GCP Billing + Dokmee" : "Billing GCP + Dokmee", shade: C.orange }, { a: 1, b: 2, t: en ? "Sync Agent" : "Sync Agent", shade: C.orange }] },
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', 'Segoe UI', system-ui, sans-serif", background: C.bg, color: C.text, minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center" }}>
      <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700;800&family=Playfair+Display:wght@700;800&display=swap" rel="stylesheet" />
      <div style={{ position: "fixed", inset: 0, background: "radial-gradient(ellipse at 30% 20%, rgba(47,143,158,0.06) 0%, transparent 60%), radial-gradient(ellipse at 80% 80%, rgba(212,168,67,0.04) 0%, transparent 50%)", pointerEvents: "none" }} />
          ...
          <div style={{ display: "flex", gap: "3px" }}>
            {titles.map((t, i) => (
              <button key={i} onClick={() => setPage(i)}
                style={{ padding: "5px 10px", fontSize: "9.5px", fontWeight: page === i ? 700 : 500, borderRadius: "5px", border: "none", cursor: "pointer", transition: "all 0.2s",
                  background: page === i ? `${C.teal}` : "transparent",
                  color: page === i ? C.white : C.textDim,
                }}>
                {t}
              </button>
            ))}
      ...
      {/* Page Navigation Dots + Prev/Next */}
